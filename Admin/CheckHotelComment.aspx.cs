...
public partial class Admin_CheckHotelComment : System.Web.UI.Page
{
    ...
    protected void FillGridView()
    {
        ...
        try
        {
            SqlCommand cmd = new SqlCommand("SELECT  * FROM (select HotelComment.ID as aydi, HotelComment.Title as onvan,Hotel.Title as esm ,HotelComment.PostDate,1 sortby,sts  from HotelComment,Hotel where Hotel.ID=HotelComment.HotelID AND HotelComment.sts=0 union all select HotelComment.ID as aydi,HotelComment.Title as onvan,Hotel.Title as esm ,HotelComment.PostDate,2 sortby,sts  from HotelComment,Hotel where Hotel.ID=HotelComment.HotelID AND HotelComment.sts=1 ) dum ORDER   BY sortby", con);
    ...
    protected void Button5_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            SqlCommand cmd = new SqlCommand("Update HotelComment Set sts = 1 where ID = " + Session["Comment"].ToString() , con);
            con.Open();
            cmd.ExecuteNonQuery() ;
            con.Close();
    ...
    protected void LoadComment()
    {
        ...
        try
        {
            SqlCommand cmd = new SqlCommand("select HotelComment.Title as onvan,Hotel.Title as esm , watcher.PicAdd,HotelComment.Tozihat FROM HotelComment,Hotel,watcher where HotelComment.HotelID = Hotel.ID AND watcher.PostID = " + Session["Comment"].ToString() + " AND HotelComment.ID = " + Session["Comment"].ToString(), con);
    ...
    protected void Button6_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            SqlCommand cmd = new SqlCommand("Update HotelComment Set sts = 0 where ID = " + Session["Comment"].ToString(), con);
            con.Open();
            cmd.ExecuteNonQuery();
            con.Close();
            Label7.Text = "عملیات با موفقیت انجام شد";
    ...
    protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        ...
        try
        {
            ...
            string myQ = "";
            myQ += "DELETE FROM watcher WHERE PostID = " + CurrentID + ";";
            myQ += "DELETE FROM VoteHotel WHERE CommentID = " + CurrentID + ";";
            myQ += "DELETE FROM HotelComment WHERE ID = " + CurrentID + ";";
