    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
        if (CurrentPass.Text.Trim() != "" && NewPass.Text.Trim() != "" && RNewPass.Text.Trim() != "")
        {
            if (NewPass.Text.Trim() == RNewPass.Text.Trim())
            {
                ...
                try
                {
                    SqlCommand cmd = new SqlCommand("UPDATE Members Set Password = " + CheckNull(Incode(RNewPass.Text.Trim()), 1) + " WHERE Username = '" + Session["Member"].ToString() + "' AND Password = " + CheckNull(Incode(CurrentPass.Text.Trim()), 1), con);
                    con.Open();
