...
public partial class Member_viewticket : System.Web.UI.Page
{
    ...
    protected void LoadTicket()
    {
        ...
        if (v != null && v != "")
        {
            ...
            try
            {
                SqlCommand cmd2 = new SqlCommand("SELECT 1 from Ticket WHERE ID = " + v + " AND UserID = " + Session["UserID"], con);
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        if (v != null && v != "")
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("INSERT INTO TicketComment(MSG,ReplyDate,UserID,TicketID) VALUES (" + CheckNull(MMM, 1) + " , " + CheckNull((GetCurrentDate() + " " + GetCurrentTime()), 1) + "," + Session["UserID"].ToString() + " , " + v + "); UPDATE Ticket SET sts1 = N'در انتظار پاسخ' WHERE ID = " + v, con);
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
                DataList1.DataBind();
