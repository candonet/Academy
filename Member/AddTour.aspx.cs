...
public partial class Member_AddTour : System.Web.UI.Page
{
    ...
    protected void FillGridView()
    {
        ...
        try
        {
            ...
            if (flg == true)
                myQu = " AND " + myQu.Substring(1, myQu.Length - 5);
            SqlCommand cmd = new SqlCommand("select ID,Nam,(select nam From AgencyDetail WHERE UserID = TourBase.UserID) as Agency,(select TourCat From TourCat WHERE ID = TourBase.TourCatID) as Kind,modat,tarikh2 FROM TourBase WHERE UserID =" + Session["UserID"].ToString() + myQu + " Order By ID DESC", con);
    ...
    protected void CheckAllow()
    {
        ...
        try
        {
            SqlCommand cmd = new SqlCommand("select CTour from AgencyDetail WHERE UserID = " + Session["UserID"].ToString(), con);
            SqlDataReader dr = null;
            con.Open();
            dr = cmd.ExecuteReader();
            bool flg = false;
            ...
            con.Close();
            if (flg == true) Form1.InnerHtml = "متاسفانه شما مجوز تعریف تور را ندارید";
        }
        catch (Exception exp)
        {
    ...
    private void LoadLists()
    {
        ...
        try
        {
            SqlCommand cmd = new SqlCommand("SELECT ID,Nam From TourBase WHERE UserID = " + Session["UserID"]  +" order by Id ", con);
            SqlDataReader dr = null;
            con.Open();
            dr = cmd.ExecuteReader();
            if (dr.HasRows)
    ...
    protected void Button3_Click(object sender, EventArgs e)
    {
        if (DropDownList3.Text != "نامشخص" && TextBox1.Text.Trim() != "" && TextBox2.Text.Trim() != "" && TextBox8.Text.Trim() != "" && TextBox4.Text.Trim() != "" && TextBox5.Text.Trim() != "" && CityName.Text.Trim() != "" && TextBox3.Text.Trim() != "" && TextBox7.Text.Trim() != "")
        {
            ...
            try
            {
                ...
                Vals += " , " + Session["UserID"] + " , " + CheckNull(Data4[0], 0) + " , " + CheckNull(KeyWord.Text, 1) + " , " + CheckNull(Loks, 0) + " , " + CheckNull(Ofr, 0) + ");Declare @CurrentID bigint;set @CurrentID = SCOPE_IDENTITY();Update TourBase Set TourProfile = N'" + TextBox1.Text.Replace(" ", "-") + "' + '-' + CONVERT(varchar(10),@CurrentID) WHERE ID = @CurrentID";
    ...
    protected void Button4_Click1(object sender, EventArgs e)
    {
        if (DropDownList3.Text != "نامشخص" && TextBox1.Text.Trim() != "" && TextBox2.Text.Trim() != "" && TextBox8.Text.Trim() != "" && TextBox4.Text.Trim() != "" && TextBox5.Text.Trim() != "" && CityName.Text.Trim() != "" && TextBox3.Text.Trim() != "" && TextBox7.Text.Trim() != "")
        {
            ...
            try
            {
                ...
                Vals += " , az = " + CheckNull(CityName.Text, 1) + " , modat = " + CheckNull(TextBox8.Text, 1) + " , kind = " + CheckNull(TextBox2.Text, 1) + " , tarikh1 = " + CheckNull(StartDate, 1);
    ...
    private void ShowCountryItem()
    {
        ...
        try
        {
            ...
            SqlCommand cmd = new SqlCommand("select Nam,keshvar,shahr,nahie,az,modat,kind,tarikh1,tarikh2,madarek,khadamat,tozihat,ID,TourCatID,Loks,Offer,KeyW from TourBase where id =" + Session["TourID"].ToString(), con);
            SqlDataReader dr = null;
            Session["TiID"] = null;
            con.Open();
            dr = cmd.ExecuteReader();
            if (dr.HasRows)
            {
                ...
                if (dr["Loks"].ToString() == "True") DropDownList11.SelectedIndex = 1;
                else DropDownList11.SelectedIndex = 1;
                if (dr["Offer"].ToString() == "True") DropDownList10.SelectedIndex = 1;
        ...
        if ((Qeimat1.Text.Trim() != "" || Qeimat1P.Text.Trim() != "") && (Qeimat2.Text != "" || Qeimat2P.Text.Trim() != "") && (Qeimat3.Text != "" || Qeimat3P.Text.Trim() != "") && (Qeimat4.Text != "" || Qeimat4P.Text.Trim() != ""))
        {
            if ((sl1.Visible == true && DropDownList6.Text != "") || (sl1.Visible == false && HotelName.Text.Trim() != ""))
            {
                ...
                try
                {
                    ...
                    {
                        string[] Data = DropDownList6.Text.Split('-');
                        Vals = "Update TourHotel Set HotelKhademat = '" + DropDownList16.Text + "' , hotelID =" + CheckNull(Data[0], 0) + " , qeimat1=" + CheckNull(Qeimat1.Text, 1);
                        Vals += " , Qeimat1P = " + CheckNull(Qeimat1P.Text.Trim(), 1) + " , Qeimat2P = " + CheckNull(Qeimat2P.Text.Trim(), 1) + " , Qeimat3P = " + CheckNull(Qeimat3P.Text.Trim(), 1) + " , Qeimat4P = " + CheckNull(Qeimat4P.Text.Trim(), 1);
                        ...
                        Vals += " , qeimat2 = " + CheckNull(Qeimat2.Text, 1) + " , qeimat3 = " + CheckNull(Qeimat3.Text, 1) + " , qeimat4 = " + CheckNull(Qeimat4.Text, 1) + " , tozihat = " + CheckNull(N1, 1) + " WHERE  ID = " + Session["TiID"];
                    }
                    else
                    {
                        Vals = "Update TourHotel Set HotelKhademat = '" + DropDownList16.Text + "' , HotelID = NULL , HotelName =" + CheckNull(HotelName.Text, 1) + " , qeimat1=" + CheckNull(Qeimat1.Text, 1);
                        Vals += " , Qeimat1P = " + CheckNull(Qeimat1P.Text.Trim(), 1) + " , Qeimat2P = " + CheckNull(Qeimat2P.Text.Trim(), 1) + " , Qeimat3P = " + CheckNull(Qeimat3P.Text.Trim(), 1) + " , Qeimat4P = " + CheckNull(Qeimat4P.Text.Trim(), 1);
                        Vals += " , Qeimat1Pv = " + CheckNull(DropDownList17.Text.Trim(), 1) + " , Qeimat2Pv = " + CheckNull(DropDownList18.Text.Trim(), 1) + " , Qeimat3Pv = " + CheckNull(DropDownList19.Text.Trim(), 1) + " , Qeimat4Pv = " + CheckNull(DropDownList20.Text.Trim(), 1);
                        Vals += " , qeimat2 = " + CheckNull(Qeimat2.Text, 1) + " , qeimat3 = " + CheckNull(Qeimat3.Text, 1) + " , qeimat4 = " + CheckNull(Qeimat4.Text, 1) + " , tozihat = " + CheckNull(N1, 1) + " , HotelStar = " + CheckNull(DropDownList9.Text, 1) + " WHERE  ID = " + Session["TiID"];
                    }
                    string MyQu = Vals;
                    SqlCommand cmd = new SqlCommand(MyQu, con);
                    con.Open();
